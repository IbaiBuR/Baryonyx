...
TEST_SUITE("Position Tests") {
    TEST_CASE("fen parsing") {
        SUBCASE("start position") {
            ...
            CHECK(pos.piece_type_bb(piece_type::knight) == bitboards::bitboard(0x4200000000000042ULL));
            CHECK(pos.piece_type_bb(piece_type::king) == bitboards::bitboard(0x1000000000000010ULL));
            CHECK(pos.piece_type_bb(piece_type::bishop) == bitboards::bitboard(0x2400000000000024ULL));
            CHECK(pos.piece_type_bb(piece_type::rook) == bitboards::bitboard(0x8100000000000081ULL));
            CHECK(pos.piece_type_bb(piece_type::queen) == bitboards::bitboard(0x800000000000008ULL));
